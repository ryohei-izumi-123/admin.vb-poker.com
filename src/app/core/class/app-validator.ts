...
export class AppValidator {
   ...
   */
  public static isJson(ctrl: AbstractControl): ValidationErrors {
    const error: ValidationErrors = { isJson: true };
    const value: any = ctrl.value;
    if (_.isEmpty(value)) {
      return null;
    }

    return Util.isJson(value) ? null : error;
  }
   ...
   * @memberof AppValidator
   */
  public static isDecimal(ctrl: AbstractControl): ValidationErrors {
    const regex: RegExp = new RegExp(/^[-]?([1-9]\d*|0)(\.\d+)?$/);
    const error: ValidationErrors = { isDecimal: true };
    if (!_.isEmpty(ctrl.value)) {
      return null !== ctrl.value.match(regex) ? null : error;
    }
  ...
  /**
   *
   * @description 半角英大文字、半角英小文字、半角数字、記号のうち、いずれか３種類を含む８文字以上の文字列
   * @static
   * @param {AbstractControl} ctrl
   * @returns {ValidationErrors}
   * @memberof AppValidator
  ...
  public static isValidFile(ctrl: AbstractControl): ValidationErrors {
    ...
    let file: File;
    let size: number;
    if (!_.isEmpty(ctrl.value) && _.size(ctrl.value) > 0) {
   ...
   *
   * @static
   * @param {RegExp} [regex=new RegExp(/(.*?)\.(jpg|png|jpeg|gif|svg)$/)]
   * @memberof AppValidator
   */
  public static isValidFileExtension = (
    regex = new RegExp(/(.*?)\.(jpg|png|jpeg|gif|svg)$/)
  ): ValidatorFn => {
    return (ctrl: AbstractControl): ValidationErrors | null => {
